...
If you are using only `interview` skills, no `setup-matt-pocock-skills` step is required: the interview pipeline is self-contained and tracker-free — the skills only read/write `CONTEXT.md`, `docs/adr/`, `docs/task-spec.md`, and (optionally) `docs/codebase/`.
